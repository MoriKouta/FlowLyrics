...
`ShowRepeatButton`, `ShowReverseButton`, `ShowPersonalSyncButton` and `ShowVolumeButton` default to true. These additive settings need no schema migration (schema 17); absent properties keep the existing visible controls. Settings previews apply independently; explicit Cancel restores the prior snapshot, while Close/title-bar close keeps the existing save behavior. Lock/Settings have no new hide option.

Player Repeat uses GSMTC capability and observed `AutoRepeatMode`; a rejected command never sets an optimistic state. Stop After Track is an in-memory one-shot in Personal Sync. It confirms Repeat Off before arming, requires at least three seconds of continuous observations, cancels on navigation/seek/source/repeat conflicts, and pauses (or stops) the same validated session/track at the end. The next-track fallback requires the old projected end plus a fresh next-track timeline at or after that boundary. GSMTC does not expose transition reasons: indistinguishable manual navigation exactly at the natural boundary remains a best-effort limitation, as do player latency/crossfade and arming too close to the end. No volume/mute operations are used.

Personal Sync suspends editing during pending metadata and retains the same window across track/source/lyrics changes. Confirmed edits queue with their original identity; unfinished drags/holds are cancelled. The next context resolves from storage after pending writes, with revision checks rejecting stale completions. Editor population runs at Background dispatcher priority after lyric rendering. Failed writes remain queued for retry/close. Inspector Align/Nudge shift the whole timeline; Re-sync creates a local anchor.

`LocalizedUiFont` separates fixed English visual headings/technical text (`DotFont`, always bundled Flow Dots) from translated body controls (`UiFont`, existing CJK families or Segoe UI; English uses Flow Dots). Language changes never replace DotFont or the user's overlay lyric font. Resource fonts created from C# require a base URI plus a relative resource reference. `LocalizationRuntimeTests` checks resolved glyph resources, all ten languages, fixed headings, language changes and provider values that happen to equal UI keys. The optional `FLOWLYRICS_NATIVE_CAPTURE_DIR` captures actual test windows; transparent overlays get an opaque test backdrop to exclude unrelated desktop content.

`PlayerControlVisuals` owns compact utility size, spacing, border, R/S glyph placement and neutral/hover ink. Play/Pause emphasis, active accent, muted volume and disabled/pressed template states remain intentional differences. Responsive bar sizing accounts for enabled visibility options without changing the lyric viewport. Native capture needs dispatcher presentation time; bitmap rendering alone does not validate GPU composition or per-monitor DPI changes.

`EditorUiTranslations` completes the editor/search/shared labels previously limited to English/Japanese. Diagnostics payload field names, raw metadata, product names, palette names and exception details remain technical data. Visual section headings stay English; actions, help, matching summaries and cache/selection descriptions go through `LocalizationService`. Use `HasTranslation` to detect missing resources without rejecting valid identical spellings.
